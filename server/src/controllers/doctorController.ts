...
export const getDoctors = async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    const formatted = doctors.map(d => ({ ...d.toObject(), _id: d.id }));
...
export const getDoctorById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({ success: true, data: { ...doctor.toObject(), _id: doctor.id } });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Server error' });
  }
};
...
export const createDoctor = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(201).json({ success: true, data: { ...doctor.toObject(), _id: doctor.id } });
  } catch (error) {
