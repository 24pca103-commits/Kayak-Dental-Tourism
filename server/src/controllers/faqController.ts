...
export const getFAQs = async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    const formatted = faqs.map(f => ({ ...f.toObject(), _id: f.id }));
...
export const createFAQ = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(201).json({ success: true, data: { ...faq.toObject(), _id: faq.id } });
  } catch (error) {
...
export const updateFAQ = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({ success: true, data: { ...faq.toObject(), _id: faq.id } });
  } catch (error) {
